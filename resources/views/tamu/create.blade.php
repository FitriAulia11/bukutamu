        ...
        <div class="card-body">
            ...
            <form action="{{ route('form.tamu.store') }}" method="POST" autocomplete="off">
                ...
                <div class="mb-3">
                    ...
                    <input type="datetime-local" name="tanggal" id="tanggal" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', now()->format('Y-m-d\TH:i')) }}" required>
                    @error('tanggal')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
